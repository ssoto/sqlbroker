...
class SQLParser(object):
	...
	def parse(self, sqlstatements):
		...
		keywords = ('SELECT', 'FROM', 'JOIN', 'WHERE', \
					'GROUP BY', 'HAVING', 'ORDER BY', \
					'DISTINCT', 'INSERT INTO', 'UPDATE', 'DELETE FROM')
...
if __name__ == '__main__':
	...
	sqlst = """select pepe AS pepito, pp, ddd
			...
			where pppp.id1 BETWEEN 5 AND 7 and PPPP.id1 like \'%dd%\' and ppp2.id2 >= 3
			GROUP by pepe, pp, ddd HAVING s >= 3 ORDER BY pepe DESC;
